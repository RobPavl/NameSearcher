...
print "Please enter your request here: "
search_request = gets.strip
...
def search data, request
  ...
  # Checking if there is quoted symbols in request
  check_presence_of_quotes data, request

  # case for search match precision
  data.map(&:values).each {|name, type, author| @finded_names << name if name.upcase == request.upcase ||
                                                                         type.upcase.include?(request.upcase) ||
                                                                         author.upcase.include?(request.upcase)}

  # case of word order reordering of a search query
  if @finded_names.empty?
    find_data data, request
  end
  if @duplicated_names
    puts "Names found: #{@finded_names.find_all {|e| @finded_names.rindex(e) != @finded_names.index(e)}.uniq}"
  else
    puts "Names found: #{@finded_names}"
  end
end


def check_presence_of_quotes data, request
  find_data data, request.match(/(["'])(.*?[^\\])\1/).to_s.gsub(/["']/, '')
end


def find_data data, request
  request.split.each do |rec|
    data.map(&:values).each {|name, type, author| @finded_names << name if type.upcase.match(/^(?=.*#{rec.upcase})/)}
    data.map(&:values).each {|name, type, author| @finded_names << name if author.upcase.match(/^(?=.*#{rec.upcase})/)}
    data.map(&:values).each {|name, type, author| @finded_names << name if name.upcase.match(/^?(?=.*#{rec.upcase})/)}
  end
  @duplicated_names = true unless @finded_names.empty?
end
